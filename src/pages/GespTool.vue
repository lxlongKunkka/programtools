  ...
  try {
    ...
    if (isAdmin) {
        ...
        availableModels.value = allModels.filter(m => {
          if (m.id === 'gemini-3.5-flash' || m.id === 'gemini-2.5-flash') return true
          if (m.role === 'admin') return false
          return true
        })
    }
    
    // Prefer gemini-3.5-flash if available, otherwise keep default or pick first
    if (availableModels.value.find(m => m.id === 'gemini-3.5-flash')) {
