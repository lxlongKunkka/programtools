  ...
  <div class="learning-map-container" :class="{ 'is-edit-mode': editMode }">
    ...
    <div class="course-layout">
      ...
      <div class="course-content">
        <!-- Phase B: Design editor panel (edit mode) -->
        <Design
          v-if="editMode"
          ref="designer"
          embedded
...
export default {
  ...
  methods: {
    ...
    selectNode(type, data, parentLevel = null, preventExpand = false) {
      ...
      // Phase A/B: sync Design editor cursor when edit mode is active
      if (this.editMode) {
        this.editModeNode = { type, id: data._id || data.name }
      }
    },
    // Phase A/B: enter edit mode and pass current node as initial cursor
    enterEditMode() {
    ...
    goToChapter(level, chapter) {
      // Use the same status function that drives the visual state —
      // if the card shows locked, block navigation; otherwise allow.
      if (this.getChapterStatusClass(level, chapter) === 'status-locked') {
        return
      }
      ...
      this.$router.push({
        ...
        query: { lid: level ? level._id : undefined }
      })
    },
    renderMarkdown(text) {
      if (!text) return ''
...
/* Phase B: sidebar visual cue in edit mode */
.is-edit-mode .course-sidebar {
  border-right: 2px solid #6366f1;
}
.is-edit-mode .sidebar-header {
  background: #f0f0ff;
}
...
/* Phase B: Design panel fills the content area without padding */
