<template>
  <div class="atcoder-root">
    ...
    <div v-if="contestInfo" class="contest-info">
      ...
      <span class="contest-meta">共 {{ contestInfo.problems.length }} 题</span>
      <div class="bulk-actions">
        ...
        >
          {{ isProcessing ? `⏳ 处理中 (${processingLabel})` : `🚀 处理选中 (${selectedCount})` }}
        </button>
        ...
        <template v-if="currentProblem">
          <div class="detail-header">
            ...
            <div class="detail-actions">
              ...
              >
                {{ isProblemProcessing(currentProblem.taskId) ? '⏳ 处理中...' : '▶ 单独处理' }}
              </button>
              <button
            ...
            <template v-else>
              ...
              <template v-if="activeTab === 'content'">
                <div v-if="!getResult(currentProblem.taskId, 'content')" class="empty-hint">
                  ...
                  <span v-else>尚未抓取，点击"处理"开始</span>
                </div>
...
export default {
  ...
  methods: {
    ...
    async downloadAll() {
      ...
      this.showToastMessage(`✅ 已打包 ${count} 道题目`)
    },
