...
2. **必须使用以下 CSS 样式模板**（请保持核心样式不变，可根据内容微调）：
   - **严禁**修改 `body { background-color }` 和 `.slide { width / height }`——必须保持 `background-color: #fff`、`.slide { width: 100%; height: 100%; }`，不得添加 `border-radius` 或改为卡片式布局。
   - 全屏白色背景，白色幻灯片全屏卡片。
   - 响应式字体 (clamp)。
   - 底部圆形导航按钮。
...
7. **⚠️ 幻灯片切换逻辑（最重要）**：下方 \`<script>\` 中的 \`showSlide()\` 函数以及滑块/导航代码，必须**一字不差地复制**到你生成的 HTML 里，不允许自行简化或改写。这是防止切换时黑屏的关键代码。
