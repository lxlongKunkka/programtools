...
3. **结构**：
   ...
   - **题目解析页**：简明扼要的题目描述和约束条件。**必须包含输入格式、输出格式和数据范围的说明**。
     - **关键要求**：所有数学符号、变量名、公式、数据范围**必须严格使用 LaTeX 格式**（**只允许** \`$...$\` 行内公式或 \`$$...$$\` 块级公式）。**严禁使用 \`\\(...\\)\` 或 \`\\[...\\]\` 语法**，这些语法在本环境（KaTeX）无法渲染。
   ...
   - **思路探讨页（核心，必须非常详细）**：这是报告的灵魂。请**详细、深入且通俗易懂**地讲解解题思路。
     - **排版要求**：正文使用 &lt;p&gt; 标签。小标题使用 &lt;h3&gt; 或 &lt;h4&gt;，**严禁**使用 &lt;h1&gt; 或 &lt;h2&gt;，以免字体过大。**严禁使用 Markdown 加粗（**text**），请使用 <strong> 标签。**
     - **必须包含以下内容**（分多个子页展示）：
       * **第一反应**：拿到题目的第一想法是什么？（暴力枚举？直接计算？）
       * **暴力思路**：最直观的解法是什么？代码如何实现？复杂度多少？
       * **瓶颈分析**：暴力解法为什么慢？瓶颈在哪里？（用具体例子说明）
       * **正解推导（核心，必须极其详细）**：这是整个报告最重要的部分，**绝对不能省略或一笔带过**。要求：
         - **关键观察**：从暴力出发，具体说明发现了什么规律/性质/数学推导，使得时间复杂度可以降低。必须用文字和例子共同说明，而不只是给结论。
         - **算法设计过程**：逐步推导出正确算法的每一步思路，解释"为什么这样设计"，包括数据结构选择、状态定义、转移方程或核心操作的设计理由。
         - **正确性说明**：简要说明为什么正解是正确的（如贪心的选择性质、DP的最优子结构、二分的单调性等），不只是给出算法。
         - **复杂度分析**：说明正解的时间/空间复杂度，并与暴力对比。
         - 以上四点**必须各自独立成段**，使用 <h4> 小标题区分，每段至少 3 句话。
...
### 必须包含的 CSS 和 JS 模板
请严格基于以下代码结构生成 HTML（你可以修改内容，**但下方 \`<script>\` 中的 \`showSlide()\` 函数及滑块/导航代码必须一字不差地复制，不允许简化或改写，这是防止切换黑屏的关键**）。
**特别注意**：
...
<head>
    ...
    <style>
        /* --- 基础和布局 --- */
        html, body { height: 100%; margin: 0; overflow: hidden; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; background-color: #fff; }
