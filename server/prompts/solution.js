...
**排版要求**：
1. 所有数学公式、变量、数字必须使用 LaTeX 格式，**只允许** \`$...$\`（行内公式）或 \`$$...$$\`（块级公式），例如 \`$O(n)$\`、\`$a[i]$\`。**严禁使用 \`\\(...\\)\` 或 \`\\[...\\]\` 语法**，这些语法在本环境（KaTeX）中无法渲染。
2. **严禁在 LaTeX 公式（$ ... $）内部直接使用中文**。如果需要中文说明，请使用 \`\\text{中文}\` 或者将中文移出公式环境。例如：不要写 $x 的值$，要写 $x$ 的值。
3. 语言生动有趣，避免枯燥的堆砌术语。
4. 逻辑清晰，层层递进。
5. **再次强调**：第 4 节的代码块必须被 \`<!-- AC_CODE_START -->\` 和 \`<!-- AC_CODE_END -->\` 包裹，这两行是系统自动提取代码的唯一依据，缺少任意一行将导致代码提取失败。`
