...
3. **对所有调用 randint(a, b) 的地方**，确认 a <= b 一定成立，否则加 max/min 保护。
...
14. **【⚠️ 总和约束：必须用预算分配法，禁止先随机再事后补救】**
   当题目有"多组测试，$\\sum N \\le L$"这类总和限制时，**独立随机每个 N 再在最后统一检查总和的写法是错的**，期望值接近上限时极大概率超限。
   
   **正确做法：预算分配法**（Budget Allocation）：
   \`\`\`python
   def gen_sizes(T, min_N, max_N, sum_limit):
       """在 sum_limit 约束下随机生成 T 个 N，每个 N ∈ [min_N, max_N]"""
         if T * min_N > sum_limit:
            raise ValueError("min_N 过大，无解")
       sizes = [min_N] * T
   ...
   **本质**：先把每个槽填到 min_N（保底），再随机分配剩余预算，从根本上保证 ∑N ≤ sum_limit。
   - ❌ 错误写法：\`N = randint(lo, hi)\` × T 次，然后最后才发现 \`sumN > limit\`
...
16. **【⚠️ 数据验证必须在生成时进行，禁止只在最后统一检查】**
   把所有合法性检查堆到循环结束后统一处理的写法是错的：一旦出错，已生成的部分数据已写入文件，且错误信息无法定位到具体生成逻辑。
   
   **正确做法：在每个元素/参数确定时就从范围设计上保证合法**：
   \`\`\`python
   # ✅ 生成参数时直接选合法范围
   N = randint(1, 100)
   ...
   # 无需额外校验，因为 randint(1, N) 本身就保证了范围
   \`\`\`
   
   如果元素来源复杂（如手动构造），应在构造逻辑里直接限制取值范围，而不是等所有测试点都生成完再统一检查。
