...
router.post('/generate-tags', authenticateToken, checkModelPermission, async (req, res) => {
  try {
    ...
    1. 分析题目核心算法，选出：一个等级标签（gesp1~gesp10）+ 1~3 个知识点标签。
    2. 根据题目内容生成一个简洁的中文标题（不超过20字）。

    等级说明：gesp1-8 对应 GESP 认证各级（由易到难），gesp9 对应 CSP-S/提高级，gesp10 对应 NOI 及以上。

    等级标签（必选1个）：${JSON.stringify(LEVEL_TAGS)}

    知识点标签（选1-3个）：${JSON.stringify(KNOWLEDGE_TAGS)}
