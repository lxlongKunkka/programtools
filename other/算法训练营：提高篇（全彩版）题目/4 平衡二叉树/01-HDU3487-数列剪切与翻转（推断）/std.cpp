...
void Splay(int x,int goal){//将x旋转为goal的下方
    while(tr[x].fa!=goal){
        int y=tr[x].fa,z=tr[y].fa;
        if(z!=goal)
...
void Build(int l,int r,int &t,int fa){//创建伸展树 
	if(l>r)
		return;
	int mid=l+r>>1;
	t=New(fa,mid);
...
void Cut(int l,int r,int c){//区间切割[l,r],插入第c个之后 
...
void Flip(int l,int r){//区间翻转[l,r]
	int x=Findk(root,l-1),y=Findk(root,r+1);
	Splay(x,0),Splay(y,x);
